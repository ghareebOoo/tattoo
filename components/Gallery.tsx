  ...
  return (
    <motion.div variants={variants} initial="hidden" whileInView="show" className="bg-[#f9f9f9] p-5 mt-5">
      ...
      <motion.div
        ...
        whileInView={{ y: 0, opacity: 1 }}
        transition={{ duration: 1 }}
        className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5"
      >
        {dataGallery.map((item, index) => (
          ...
          >
            <Image src={item.img} alt={`gallery-image-${index}`} width={500} height={500} className="w-full h-full object-cover" />
          </div>
        ))}
      </motion.div>
      ...
      <motion.button
        ...
        whileInView={{ y: 0, opacity: 1 }}
        transition={{ duration: 1 }}
        className="cursor-pointer uppercase mx-auto mt-8 bg-dark flex items-center justify-center gap-4 text-white px-6 py-4"
      >
        View all <FaArrowRight className="text-2xl" />
      </motion.button>

      {showSwiper && selectedIndex !== null && (
              ...
              <SwiperSlide key={index} className="relative">
                ...
                <div className="p-5 w-full h-full flex justify-between items-center relative">
                  ...
                  <div className="w-[500px] mx-auto">
                    <Image src={item.img} alt={`swiper-image-${index}`} width={500} height={500} className="w-full rounded-xl" />
                  </div>
